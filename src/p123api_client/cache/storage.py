...
class SQLiteStorage:
    ...
    def store(self, key: str, data: Any, endpoint: str,
        ...
        try:
            ...
            with self.get_connection() as conn:
                ...
                """, (
                    ...
                    serialized,
                    endpoint,
                    datetime.now(timezone.utc).replace(microsecond=0),  # Store as native datetime
                    expires_at.astimezone(timezone.utc).replace(microsecond=0),  # Store as native datetime
                    len(serialized)
                ))
                conn.commit()
    ...
    def retrieve(self, key: str) -> Tuple[Optional[Any], Optional[Dict]]:
            ...
            with self.get_connection() as conn:
                ...
                if row and row[0]:
                    ...
                    if current_size_bytes > max_size_bytes:
                        logger.info(f"Cache size ({current_size_bytes/1024/1024:.2f}MB) exceeds limit "
                                   f"({max_size_bytes/1024/1024:.2f}MB). Cleaning up...")
                        
                        # Delete oldest entries first, based on last_accessed
                        # Calculate how much we need to remove
                        excess_bytes = current_size_bytes - (max_size_bytes * 0.8)  # Remove enough to get to 80% of max
                        ...
                        if entries_to_delete:
                            ...
                            logger.info(f"Removed {len(entries_to_delete)} entries "
