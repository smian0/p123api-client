...
class BacktestRequest(BaseModel):
    ...
    screen_type: Optional[ScreenType] = Field(ScreenType.STOCK, description="Type of screen (stock/etf/fund)")
    screen_rules: Optional[List[ScreenRule]] = Field(None, description="List of screen rules")
    universe: Optional[str] = Field(None, description="Universe for backtest (e.g., SP500)")
    max_num_holdings: Optional[int] = Field(None, description="Maximum number of holdings")
    benchmark: Optional[str] = Field(None, description="Benchmark for comparison (e.g., SPY)")
    
    # Date fields
    start_dt: Optional[date] = Field(None, description="Start date for backtest")
    end_dt: Optional[date] = Field(None, description="End date for backtest")
    start_date: Optional[date] = Field(None, description="Alternative start date field")
    end_date: Optional[date] = Field(None, description="Alternative end date field")
    
    # Method fields
    screen_method: Optional[ScreenMethod] = Field(
    ...
    rebal_freq: Optional[Any] = Field(None, description="Rebalancing frequency")
    trans_price: Optional[Any] = Field(None, description="Transaction price")
    risk_stats_period: Optional[Any] = Field(None, description="Risk stats period")
    screen_params: Optional[Any] = Field(None, description="Screen parameters")
    
    # Multi-factor fields
    factors_tsv_path: Optional[str] = Field(None, description="Path to TSV file with factors")
